...
namespace PascalDotNet {
    ...
    public class Tokenizer {
        ...
        private Boolean IsWordSymbol(string s) {
            foreach (string ws in wordSymbols) {
                if (s.ToLower() == ws.ToLower()) {
                    return true;
                }
            }
            return false;
        ...
        private Boolean ExtractWordSymbol(ref Token token, StringCollection code, int line, ref int column) {
            ...
            if (ExtractIdentifier(ref t, code, line, ref col) && IsWordSymbol(t.Text)) {
